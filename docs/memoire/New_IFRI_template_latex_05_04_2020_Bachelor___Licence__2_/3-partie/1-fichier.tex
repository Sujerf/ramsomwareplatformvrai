...
Ce chapitre décrit la mise en \oe{}uvre de RansomShield, les scénarios de validation conduits et les résultats obtenus. La démarche de validation s'inspire des recommandations du NIST pour la gestion des incidents de sécurité \cite{nistIncidentResponse2024}. Il présente également les limites identifiées et les pistes d'amélioration envisagées.
...
\begin{itemize}
    \item \textbf{agents}, \textbf{events}, \textbf{alerts}, \textbf{incidents} : c\oe{}ur du pipeline de détection ;
...
\noindent La Figure~\ref{fig:liste-agents} montre les trois agents enrôlés lors des tests. L'agent \textbf{rs-fileserver} (\texttt{10.20.0.11}) est celui qui a généré les événements du scénario~3 (extension \texttt{.locked}, score~135)~; c'est donc à cet agent qu'est rattaché l'incident présenté dans les figures suivantes.
...
L'accès à la console est protégé par une authentification à deux facteurs TOTP \cite{anssiMFA2021}. En cas de perte de l'application d'authentification, dix codes de secours à usage unique sont disponibles. Deux rôles sont définis~: les analystes accèdent à la supervision et aux opérations courantes, les administrateurs disposent en plus des droits de configuration, de gestion des utilisateurs et de lancement des simulations. Ce cloisonnement est appliqué au niveau des routes Laravel et des politiques Eloquent.
...
Les scénarios de validation vérifient le bon fonctionnement de chaque maillon de la chaîne de traitement, de la collecte sur le terminal jusqu'à l'affichage dans la console. Les tests ont été réalisés dans un répertoire dédié sur l'agent \textbf{rs-fileserver} (adresse~: \texttt{10.20.0.11}), avec l'agent actif et la plateforme en cours d'exécution. Cet agent est utilisé de manière cohérente dans toutes les figures du présent chapitre.

\begin{table}[htbp]
\centering
\caption{Synthèse des scénarios de validation (agent~: \textbf{rs-fileserver}, \texttt{10.20.0.11})}
...
\hline
1 & Fichier normal & Création d'un fichier \texttt{.txt} dans le dossier surveillé de rs-fileserver & Niveau normal, aucune alerte générée. \\
\hline
2 & Renommage simple & Renommage d'un fichier \texttt{.doc} en \texttt{.bak} sur rs-fileserver & Score de risque augmenté, signal renommage enregistré. \\
\hline
3 & Extension suspecte & Renommage vers l'extension \texttt{.locked} sur rs-fileserver & Score~135, niveau critique~; alerte et incident créés et rattachés à rs-fileserver. \\
\hline
4 & Génération d'alerte & Le score de rs-fileserver dépasse le seuil critique configuré (80) & Alerte visible avec score~135, niveau critique et signaux détectés. \\
\hline
5 & Incident et action & Alerte critique déclenchée depuis rs-fileserver & Incident ouvert, cinq actions de protection proposées pour rs-fileserver. \\
\hline
6 & Kill chain complète & Simulation \texttt{ransomware\_full} (22 événements) sur rs-fileserver & Alertes critiques, incident actif, actions en cascade~; toutes rattachées à rs-fileserver. \\
\hline
7 & Isolation + rollback & Approbation de l'isolation réseau de rs-fileserver, puis rollback & Règles iptables appliquées sur rs-fileserver puis supprimées~; statuts mis à jour. \\
\hline
8 & Mode automatique & Action de scan configurée en exécution automatique pour rs-fileserver & Action déclenchée et exécutée sans intervention~; résultat consigné dans le journal d'audit. \\
\hline
9 & Mode manuel & Action de scan déclenchée manuellement depuis la console sur rs-fileserver & Action exécutée à la demande~; statut mis à jour en temps réel dans la file d'approbation. \\
\hline
\end{tabularx}
\end{table}
