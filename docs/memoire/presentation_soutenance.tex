...
\setbeamertemplate{background}{%
  \begin{tikzpicture}[overlay,remember picture]
    ...
    \end{scope}
    % Barre accent verticale gauche (couverte sur les slides plain/section)
    \fill[accent]
      (current page.south west) rectangle
      ([xshift=0.26cm]current page.north west);
  \end{tikzpicture}%
}
...
% Slide séparateur de section — panneau bleu gauche + titre à droite
\newcommand{\sectionslide}[2]{%
  \begin{frame}[plain,noframenumbering]%
    \begin{tikzpicture}[overlay,remember picture]
      % Fond blanc
      \fill[white](current page.north west) rectangle (current page.south east);
      % Panneau bleu gauche (38 % de la largeur = 6.1 cm sur 16 cm)
      \fill[primary]
        (current page.south west) rectangle
        ([xshift=6.1cm]current page.north west);
      % Liseré accent bleu clair en haut (pleine largeur)
      \fill[accent]
        (current page.north west) rectangle
        ([yshift=-0.20cm]current page.north east);
      % Fine bande dprimary sous le liseré
      \fill[dprimary]
        ([yshift=-0.20cm]current page.north west) rectangle
        ([yshift=-0.36cm]current page.north east);
      % Filet séparateur vertical (accent) entre les deux panneaux
      \draw[accent, line width=1.8pt]
        ([xshift=6.1cm, yshift=0.68cm]current page.south west) --
        ([xshift=6.1cm, yshift=-0.36cm]current page.north west);
      % Numéro fantôme (décoratif, très transparent)
      \node[font=\fontsize{108}{108}\selectfont\bfseries,
            text=white, opacity=0.09, anchor=center]
        at ([xshift=3.05cm, yshift=4.68cm]current page.south west) {#1};
      % Label "PARTIE" au-dessus du numéro
      \node[font=\fontsize{7.5}{7.5}\selectfont\bfseries,
            text=white!65, anchor=south]
        at ([xshift=3.05cm, yshift=5.8cm]current page.south west)
        {\MakeUppercase{Partie}};
      % Numéro de section (premier plan)
      \node[font=\fontsize{60}{60}\selectfont\bfseries\color{white},
            anchor=center]
        at ([xshift=3.05cm, yshift=4.68cm]current page.south west) {#1};
      % Titre de section (panneau droit, centré verticalement)
      \node[font=\fontsize{22}{30}\selectfont\bfseries\color{dprimary},
            anchor=center, text width=9.2cm, align=center]
        at ([xshift=11.05cm, yshift=4.68cm]current page.south west) {#2};
      % Pied-de-page bleu
      \fill[primary]
        (current page.south west) rectangle
...
\begin{frame}[plain,noframenumbering]
  \begin{tikzpicture}[overlay,remember picture]
    % ── Fonds ────────────────────────────────────────────────────────────
    % Panneau gauche dprimary (45 %)
    \fill[dprimary]
      (current page.south west) rectangle
      ([xshift=7.2cm]current page.north west);
    % Panneau droit blanc
    \fill[white]
      ([xshift=7.2cm]current page.south west) rectangle
      (current page.north east);
    % Hachures diagonales décoratives sur le panneau gauche
    \begin{scope}
      \clip (current page.south west) rectangle ([xshift=7.2cm]current page.north west);
      \foreach \k in {-2,-1,...,11}{%
        \draw[white,opacity=0.045,line width=9pt]
          ([xshift=\k cm]current page.south west) --
          ([xshift=\k cm + 9.5cm, yshift=9.5cm]current page.south west);
      }
    \end{scope}
    % ── Liseré accent + bande dprimary en haut ────────────────────────────
    \fill[accent]
      (current page.north west) rectangle
      ([yshift=-0.22cm]current page.north east);
    \fill[primary!60]
      ([yshift=-0.22cm]current page.north west) rectangle
      ([yshift=-0.38cm]current page.north east);
    % ── Séparateur vertical accent ────────────────────────────────────────
    \draw[accent, line width=2pt]
      ([xshift=7.2cm, yshift=0.68cm]current page.south west) --
      ([xshift=7.2cm, yshift=-0.38cm]current page.north west);
    % ── Pied-de-page ─────────────────────────────────────────────────────
    \fill[primary]
      (current page.south west) rectangle
      ([yshift=0.68cm]current page.south east);
    \node[anchor=west,font=\tiny\color{white}]
    ...
    \node[anchor=east,font=\tiny\color{white}]
      at ([xshift=-8pt,yshift=0.34cm]current page.south east)
      {24 juin 2026};
    % ── Panneau gauche — logo UAC + titre ────────────────────────────────
    \node[anchor=north, inner sep=0pt]
      at ([xshift=3.6cm, yshift=-0.65cm]current page.north west)
      {\includegraphics[height=1.20cm]{logoUAC1}};
    \node[text=white, align=center, text width=6.2cm,
          font=\fontsize{11.5}{17}\selectfont\bfseries, anchor=center]
      at ([xshift=3.6cm, yshift=4.9cm]current page.south west)
    ...
    \draw[accent, line width=1.4pt]
      ([xshift=0.9cm, yshift=3.1cm]current page.south west) --
      ([xshift=6.3cm, yshift=3.1cm]current page.south west);
    % Icône sécurité
    \node[text=accent!80, font=\fontsize{22}{22}\selectfont, anchor=center]
      at ([xshift=3.6cm, yshift=2.4cm]current page.south west)
      {\faLock};
    % ── Panneau droit — logo IFRI + auteur ───────────────────────────────
    \node[anchor=north east, inner sep=8pt]
      at ([yshift=-0.45cm]current page.north east)
      {\includegraphics[height=1.40cm]{logoIfri}};
    \node[text=textdark, font=\large\bfseries, anchor=center]
      at ([xshift=11.6cm, yshift=5.1cm]current page.south west)
      {Codjo Fréjus Loïc SANGRONIO};
    \node[text=muted, font=\small, anchor=center]
      at ([xshift=11.6cm, yshift=4.5cm]current page.south west)
      {Licence Informatique \textbar\ Option Sécurité Informatique};
    \node[text=primary, font=\small\bfseries, anchor=center]
      at ([xshift=11.6cm, yshift=4.0cm]current page.south west)
      {IFRI-UAC};
    \node[text=dprimary, font=\large\bfseries, anchor=center]
      at ([xshift=11.6cm, yshift=3.2cm]current page.south west)
      {24 juin 2026};
    % Encadreur
    \node[anchor=south west, font=\scriptsize, text=textdark, align=left]
      at ([xshift=7.4cm, yshift=0.82cm]current page.south west)
