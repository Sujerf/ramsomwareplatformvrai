...
\note{
  ...
  perspectives et terminerai par une démonstration en conditions réelles.
}
...
\note{
  ...
  qu'une réponse automatique sans validation humaine cause elle-même des dommages.
}
...
\note{
  ...
  30 secondes. Et sixièmement, valider l'ensemble sur des scénarios d'attaque représentatifs.
}
...
\note{
  ...
  on peut réagir avant que les dommages soient irréversibles.
}
...
\note{
  ...
  réelles, et avec un contrôle humain intégré à chaque étape.
}
...
\note{
  ...
  Le système n'agit jamais seul : chaque intervention est soumise à une validation humaine.
}
...
\note{
  ...
  le moindre malware réel — ce qui a permis de valider le système en toute sécurité.
}
...
\note{
  ...
  la configuration, ce qui garantit l'intégrité du système.
}
...
\note{
  ...
  de passe, l'utilisateur doit saisir un code temporaire valide 30 secondes seulement.
}
...
\note{
  ...
  Le système attend ensuite la décision de l'opérateur.
}
...
\note{
  ...
  pour s'assurer qu'aucune alerte ne passe inaperçue, quelle que soit sa situation.
}
...
\note{
  ...
  normaux et signale les anomalies avec encore plus de précision.
}
...
\note{
  \textbf{[MERCI — MOT DE CLÔTURE]}\\[4pt]
