...
% ─── Filigrane : cercles élégants — centres décalés à l'intérieur des coins
%     Les cercles complets créent de belles courbes traversant le slide,
%     bien plus raffinés que de simples arcs coupés à angle droit.
\setbeamertemplate{background}{%
  \begin{tikzpicture}[overlay,remember picture]
    % Groupe bas-droit : centre 2 cm à l'intérieur du coin SE
    \draw[primary,opacity=0.068,line width=2.0pt]
      ([xshift=-2cm,yshift=2cm]current page.south east) circle (5.6cm);
    \draw[primary,opacity=0.042,line width=1.3pt]
      ([xshift=-2cm,yshift=2cm]current page.south east) circle (9.0cm);
    \draw[primary,opacity=0.020,line width=0.7pt]
      ([xshift=-2cm,yshift=2cm]current page.south east) circle (12.4cm);
    % Accent haut-gauche : centre 2 cm à l'intérieur du coin NW
    \draw[primary,opacity=0.048,line width=1.4pt]
      ([xshift=2cm,yshift=-2cm]current page.north west) circle (4.4cm);
    \draw[primary,opacity=0.024,line width=0.7pt]
      ([xshift=2cm,yshift=-2cm]current page.north west) circle (7.2cm);
  \end{tikzpicture}%
}
...
\begin{frame}[plain,noframenumbering]
  \begin{tikzpicture}[overlay,remember picture]
    ...
    \fill[dprimary]
      (current page.south west) rectangle
      ([yshift=0.40cm]current page.south east);
    % Grande inscription blanche centrée
    \node[anchor=center,font=\fontsize{34}{34}\selectfont\bfseries,text=white]
      at ([yshift=1.4cm]current page.center)
      {Merci de votre attention\,!};
    % Séparateur fin blanc
    \draw[white,opacity=0.45,line width=0.8pt]
      ([xshift=-6cm,yshift=0.35cm]current page.center) --
      ([xshift= 6cm,yshift=0.35cm]current page.center);
    % Identité
    \node[anchor=center,font=\large\bfseries,text=white]
      at ([yshift=-0.5cm]current page.center)
      {Codjo Fréjus Loïc SANGRONIO};
    \node[anchor=center,font=\normalsize,text=white!80]
      at ([yshift=-1.15cm]current page.center)
      {\texttt{lsangronio@gmail.com} \textbar{} IFRI-UAC · 2023--2024};
    \node[anchor=center,font=\small,text=white!65]
      at ([yshift=-1.75cm]current page.center)
