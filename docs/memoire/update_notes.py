...
r"""\textbf{[OUVERTURE]}\\[4pt]
  Monsieur le Président du jury, Mesdames et Messieurs les membres du jury, bonjour.\\[4pt]
  Permettez-moi, avant toute chose, de vous exprimer ma profonde reconnaissance
  pour avoir répondu présents à ce rendez-vous. Ce jour revêt pour moi un cachet
  tout particulier : il est l'aboutissement de ces années de formation, et un moment
  qui restera gravé dans mon parcours.\\[4pt]
  C'est avec fierté et humilité que je vous présente aujourd'hui RansomShield —
  un système que j'ai conçu et mis en place de bout en bout pour détecter et contrer
...
r"""\textbf{[SOMMAIRE]}\\[4pt]
  Monsieur le Président du jury, Mesdames et Messieurs les membres du jury,\\[4pt]
  Ma présentation s'articule en cinq parties. Nous partirons du contexte et de la
  problématique qui ont motivé ce travail, pour aller vers un état des lieux des solutions
  existantes, puis la conception et la réalisation du système. Je conclurai par les
  perspectives et terminerai par une démonstration en conditions réelles.\\[6pt]
...
r"""\textbf{[CHOIX TECHNIQUES]}\\[4pt]
  ...
  Pour le serveur et la console, j'ai choisi Laravel — un cadre de développement
...
r"""\textbf{[TESTS ET VALIDATION]}\\[4pt]
  ...
  J'ai adopté une démarche en trois étapes complémentaires.\\[4pt]
  Premièrement, j'ai défini cinq scénarios d'attaque représentatifs, couvrant
  ...
  Deuxièmement, j'ai mis en place un environnement de test réel sur trois machines
  virtuelles en réseau isolé, pour reproduire les conditions d'un vrai parc informatique,
  sans jamais déployer de malware réel.\\[4pt]
  Troisièmement, pour chaque scénario, j'ai mesuré trois indicateurs précis :
  ...
  de cinq secondes, zéro événement perdu. Je note honnêtement une limite :
...
r"""\textbf{[CONCLUSION]}\\[4pt]
  Nous voilà dans la cinquième et dernière partie de notre exposé.\\[4pt]
  Monsieur le Président du jury, Mesdames et Messieurs les membres du jury,
  permettez-moi de vous présenter la synthèse de ce travail.\\[4pt]
  ...
  Il est opérationnel, démontrable, et construit sans aucun malware réel.\\[4pt]
  Je suis conscient que ce travail, mené dans un cadre académique avec des ressources
  et un temps limités, n'est pas exempt de lacunes. Les tests ont été conduits sur Linux
  uniquement, et les communications restent en HTTP. Ce sont des limites que j'assume
  pleinement et que j'ai tenu à nommer clairement.\\[4pt]
  C'est pourquoi je compte sur vos observations, vos critiques et vos recommandations
  pour enrichir ce travail. Chaque remarque du jury est pour moi une opportunité
  de progresser et de mieux faire.\\[4pt]
  ...
  normaux et signale les anomalies avec encore plus de précision.\\[6pt]
  \textcolor{transition}{$\hookrightarrow$ Je reste à votre disposition pour vos questions.}""",

# ── 17. MERCI / QUESTIONS DU JURY ─────────────────────────────────────────
r"""\textbf{[MERCI — MOT DE CLÔTURE]}\\[4pt]
  Un mot pour finir, avant les échanges avec le jury.\\[4pt]
  Dans un monde où les systèmes informatiques automatisent de plus en plus
  nos décisions, RansomShield rappelle une conviction simple :
  la machine peut détecter, mais c'est l'humain qui doit décider.
  Cette philosophie n'est pas une contrainte technique — c'est un choix éthique.\\[4pt]
  Merci, merci pour votre aimable attention et pour le temps précieux
  que vous avez bien voulu accorder à ce travail.\\[8pt]
  \textbf{[POINTS DE RÉPONSE AU JURY]}\\[4pt]
  \textbullet\ \textbf{Sur le choix de Python :}
  Python dispose de bibliothèques de surveillance système natives, très légères,
  qui fonctionnent sur Windows, Linux et macOS sans modification. Choix de portabilité
  et de rapidité de développement.\\[3pt]
  \textbullet\ \textbf{Sur les fausses alertes :}
  Les seuils sont configurables. Et même quand une alerte est levée, l'analyste
  valide ou rejette avant toute action. Le système ne peut pas agir seul.\\[3pt]
  \textbullet\ \textbf{Sur l'absence de HTTPS :}
  Limite assumée du cadre académique. Le chiffrement des communications est la
  première perspective pour un déploiement en production.\\[3pt]
  \textbullet\ \textbf{Sur la différence avec un antivirus :}
  Un antivirus reconnaît les menaces par leur empreinte. RansomShield observe
  ce que le programme fait, pas ce qu'il est — les variantes inédites sont couvertes.\\[3pt]
  \textbullet\ \textbf{Sur les tests Windows :}
  Limite assumée : tests conduits sur Linux uniquement.
  L'extension à Windows est la première perspective d'évolution.\\[3pt]
  \textit{Garder les \textbf{captures d'écran et annexes} prêtes.}""",
]
